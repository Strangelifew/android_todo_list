...
class StatusDetailsActivity : AppCompatActivity() {
    private var status: StatusDBO? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.hasExtra(EXTRA_STATUS)) {
            status = intent.getParcelableExtra<StatusDBO>(EXTRA_STATUS)!!.also {
        ...
        when (item.itemId) {
            ...
            R.id.action_save -> {
                if (!statusName.text.isNullOrEmpty()) {
                    ...
                    if (intent.hasExtra(EXTRA_STATUS)) {
                        statusUseCases.updateStatus(status.toEntity())
                    } else {
                        statusUseCases.addStatus(status.toEntity())
                    }
                    finish()
                }
            }
    ...
    companion object {
        ...
        @JvmStatic
        fun startStatusDetails(caller: Activity, status: StatusDBO?, sortOrder: Int) {
